...
def transform_spatialtransform(image, width, height):
    ...
    if h * width == w * height:
        h_resize = height
        w_resize = width
    elif h / height > w / width:
        h_resize = int(width * h / w)
        w_resize = width
    else:
        h_resize = height
        w_resize = int(height * w / h)

    print("input image shape ", image.size[1], image.size[0], " resized to ", h_resize, w_resize),
    # print("height width  ", height, width)
    spatial_transform = Compose([
        Resize(size=(h_resize,w_resize)),
        CenterCrop(size=(height, width)),
    ])

    image=spatial_transform(image)

    # print("resize_shape ", h_resize, w_resize)
    # print("after_shape ", image.size[1], image.size[0])
    return spatial_transform(image)
...
def sample(
    ...
    if path.is_file():
        ...
        all_img_paths = sorted(
            [
                ...
                if f.is_file() and f.suffix.lower() in [".jpg", ".jpeg", ".png", ".JPG"]
            ]
        )
        if len(all_img_paths) == 0:
            raise ValueError("Folder does not contain any images.")
    else:
        print("No such file or dictionary", path)
        raise ValueError
    ...
    for no, input_img_path in enumerate(all_img_paths):
        ...
        for RT_idx in range(len(RTs)):
            ...
            # print("res path ", os.path.join(filepath, filename+'_'+cur_pose_name+'_frame15.png'))
            # print("res path ", os.path.join(filepath, filename+'_'+cur_pose_name+'.mp4'))
            print(f'--pose: {cur_pose_name} ...')
            if os.path.exists(os.path.join(filepath, filename+'_'+cur_pose_name+'.mp4')):
                print(f"Already down {os.path.join(filepath, filename+'_'+cur_pose_name+'.mp4')}")
                continue
            ...
            with Image.open(input_img_path) as image:
                ...
                if transform:
                    image = transform_spatialtransform(image, width=width, height=height)
                if resize:
