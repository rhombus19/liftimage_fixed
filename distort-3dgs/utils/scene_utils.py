...
def render_training_image(scene, gaussians, viewpoints, render_func, pipe, background, stage, iteration, time_now, dataset_type):
    def render(gaussians, viewpoint, path, scaling, cam_type):
        ...
        gt_depth = viewpoint.invdepthmap.permute(1,2,0).cpu().numpy()

        gt_depth = 1.0 / (gt_depth + 1e-10)
        gt_depth[gt_depth >= 1e9] = 0.0

        image_np = image.permute(1, 2, 0).cpu().numpy()  # 转换通道顺序为 (H, W, 3)
        depth_np = depth.permute(1, 2, 0).cpu().numpy()
        
        min_all = min(depth_np.min(), gt_depth.min())
        max_all = gt_depth.max().item()
        depth_np[depth_np >= max_all] = max_all
        # print("depth_np.min() ", depth_np.min(), "depth_np.max() ", depth_np.max())
        # print("gt_depth.min() ", gt_depth.min(), "gt_depth.max() ", gt_depth.max())

        depth_np -= min_all
        depth_np /= (max_all - min_all)

        gt_depth -= min_all
        gt_depth /= (max_all - min_all)
