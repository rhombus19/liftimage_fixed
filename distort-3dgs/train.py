...
from utils.loss_utils import l1_loss, ssim, l2_loss, lpips_loss, depth_ranking_loss
...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    check_params_deform = [("feature_out",gaussians._deformation.deformation_net.feature_out), 
                ("pos_deform",gaussians._deformation.deformation_net.pos_deform), 
                ("scales_deform",gaussians._deformation.deformation_net.scales_deform),
                ("rotations_deform", gaussians._deformation.deformation_net.rotations_deform), 
                ("opacity_deform",gaussians._deformation.deformation_net.opacity_deform), 
                ("shs_deform", gaussians._deformation.deformation_net.shs_deform),
                ]
    deformation_parameter_record = None
    for iteration in range(first_iter, final_iter+1):
        deformation_parameter_record = copy.deepcopy(gaussians._deformation.deformation_net)        
        ...
        for viewpoint_cam in viewpoint_cams:
            ...
            dx, ds, dr, do, dshs, time_emb = render_pkg["dx"], render_pkg["ds"], render_pkg["dr"], render_pkg["do"], render_pkg["dshs"], render_pkg["time"]
            ...
            # dptanything_depth = viewpoint_cam.depth.cuda()
            dptanything_depth = viewpoint_cam.invdepthmap.cuda()

            gt_images.append(gt_image.unsqueeze(0))
            radii_list.append(radii.unsqueeze(0))

            depths.append(depth.unsqueeze(0))
            dptanything_depths.append(dptanything_depth.unsqueeze(0))
            depth_mask_list.append(viewpoint_cam.depth_mask.unsqueeze(0))
        ...
        if stage == "fine" and time_emb is not None and math.isclose(time_emb[0,0], 0.5, abs_tol=1e-6) and math.isclose(time_emb[0,1], 0.5, abs_tol=1e-6):
            ...
            loss_zerocanonical = l2_loss(dx, dx_zero) + l1_loss(ds, ds_zero) + l1_loss(dr, dr_zero)
        else:
            loss_zerocanonical = 0.0
        ...
        if depth_l1_weight(iteration) > 0 and viewpoint_cam.depth_reliable:
            ...
            mono_invdepth = monodepth_gt.cuda()
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}",
                                          "psnr": f"{psnr_:.{2}f}",
                                          "depth": f"{Ll1depth:.{4}f}",
                                          "canonical": f"{loss_zerocanonical:.{2}f}",
                                          "point":f"{total_point}"})
            ...
            if iteration < opt.densify_until_iter :
                ...
                if  iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0 and gaussians.get_xyz.shape[0]<500000:
    ...
    # with open(os.path.join(dataset.source_path, args.json_name), 'r') as f:
    ...
    # with open(os.path.join(args.source_path, 'refined_cams.json'), 'w') as f_:
    #     json.dump(refined_cameras, f_, indent=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[1000, 1500, 2000, 3000, 3500, 4000, 4500, 5000, 7000,9000,14000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[199, 349, 999, 2999, 6999, 10999, 13999, 16000, 20000, 30_000, 45000, 60000])
